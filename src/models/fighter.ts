...
const searchFighterQuery = "SELECT * FROM fighter WHERE name ILIKE ('%' || $1 || '%')";
...
async function searchFighter(url:string, name:string) {
    const cacheKey = "__cache__" + url;
    const value = await cache.get(cacheKey);
    if (value) {
        console.log("hit cache");
    ...
    const response = await db.query(searchFighterQuery, [name]);
    cache.set(cacheKey, response); 
    return response;
}
